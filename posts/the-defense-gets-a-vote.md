...
Reading the actual curves, three things are true at once:

1. **The model's core is conservative at every horizon.** At 2 s its 10% zone catches 33% of realised positions, its 50% zone catches 74%; at 10 s, 32% and 80%. That direction of miscalibration is deliberate: the $\varepsilon$ floor, the diffusion floors, and the configuration-memory fade (the state-conditioned field [relaxes toward the neutral marginal](/posts/a-momentum-field-for-soccer/) as the current shape of play stops being informative, which fattens the long-horizon spread) all widen the density on purpose, because under-claiming your certainty is the survivable way to be wrong. What conservatism costs is sharpness, and the log-lik table already priced that in — the model wins *despite* hedging.
2. **The one thin spot is the 5 s tail.** At nominal 90% the 5 s curve covers 82%: about one ball in five lands outside what the model calls its 90% zone at that horizon (long clearances and switches of play that outrun the blended field). At 2 s and 10 s the 90% zones over-deliver (93% both). So "never overconfident" would be a lie by one curve-end; the honest statement is: conservative core everywhere, one mildly thin tail, at 5 s.
3. **Persistence's curves sit far above the model's, and that is a defect, not a virtue.** Its 10 s curve catches 74% of truth in its "10% zone" — which sounds spectacular until you remember its fitted spread at 10 s is a ~31 m Gaussian, so its "top-10% zone" is a huge blob of pitch. Coverage alone rewards vagueness; the pair (coverage, log-lik) is what can't be gamed. On the pair, the model strictly dominates: closer to the diagonal *and* several times more probability on what actually happened.

**Verdict: honest, by construction and by measurement.** The model neither lies toward confidence anywhere that matters nor needs vagueness to survive the audit; its one tail miss (82% vs 90% at 5 s) is named, small, and in the recoverable direction. This is not "as good as the null" — persistence passes the honesty audit only the way a shrug does.

### Observable 3: does the possession survive?

The transport score conditioned on the run continuing; the model's *unconditioned* output also predicts whether it continues — the surviving probability mass under the fitted turnover/out hazards is, literally, its probability that the possession is still alive $\Delta t$ from now.

A predicted probability is scored with the [Brier score](https://en.wikipedia.org/wiki/Brier_score): the squared gap between the stated probability and what happened, $(\hat p - y)^2$, averaged over anchors. Zero is clairvoyance; saying 50% about everything scores 0.25; and — because it is a proper score — the best constant strategy is to state the base rate, which is exactly what the **climatology** baseline does. Concretely: 52% of training-segment anchors saw their possession survive another 2 s (25% another 5 s, 8% another 10 s), so climatology answers every anchor with those constants and earns roughly $\bar p(1-\bar p)$. Beating it is the entire game: it is how you prove the model's *state-dependence* — pressure at the ball, contested control, pass-lane geometry — carries information about survival that the base rate doesn't have.
...
The skill column is the standard $1 - \text{Brier}_{\text{model}}/\text{Brier}_{\text{climatology}}$: the model prices survival 3–8% better than the best state-blind constant, improving with horizon, direction consistent everywhere. And every confidence interval straddles zero. Both halves of that sentence are true, and neither cancels the other: with 614 evaluation runs sharing outcomes, an effect of this size *cannot* clear a 90% interval on one match — the study is underpowered for the effect it sees, which is a statement about the data, not the model.

**Verdict: better than the null at every horizon, unproven on one match.** The consistent direction and the growth with horizon are what genuine-but-small skill looks like; they are also exactly what lucky noise can look like. Distinguishing those two takes a corpus — which is not a hedge, it is §6's entire argument, and the same standard the gate will later hold the model to against markets.
...
**Verdict: parity, once the execution is calibrated for forecasting; the decisions themselves add no measurable motion signal on this match.** The defender layer's demonstrated value is unchanged and real — it prices threat response into the hazards (§2's A/B: pressure up, clean shot generation and finishing down), and those effects ride inside the transport wins of Observable 1. As a *motion* model it is now exactly as good as knowing that defenders are heavy and cautious, and no better. What would change that verdict: a corpus (the press stratum needs ~10× the cases to resolve a small real effect), longer structural horizons (line steps and traps that inertia can't see), or decision targets entering a purpose-built motion model as one feature among several. The harness for all three now ships in the crate — `validate` runs the headline comparison; `examples/defender_fit.rs` runs the stratified version.

### So — does it work?

One table, no hedging beyond what the data forces:
...
| Is its uncertainty honest? | **Yes — conservative core, one thin tail** | 90% zones deliver 93 / 82 / 93% at 2 / 5 / 10 s; persistence is "honest" only by vagueness |
| Will the possession survive? | **Probably better than the null; unproven** | 3–8% Brier skill over climatology, every 90% CI crosses zero |
...
*Judge on trial, case 1 — a synthetic truth-reporting model. Every bubble rides the diagonal within sampling noise, ECE 0.022, and the gate correctly passes it. When a real corpus produces this picture, `calibrated: true` gets earned.*

![Judge self-test: the synthetic model that deserves to fail](/images/the-defense-gets-a-vote/reliability_overconfident.svg)
*Judge on trial, case 2 — the same corpus with the probabilities artificially sharpened toward 0 and 1. The signature of overconfidence is this exact rotation off the diagonal: low claims land above the line (it said 10%, it happened 25% of the time), high claims land below (it said 90%, it delivered 70%). ECE 0.167; the gate correctly fails it, naming reliability, the base-rate floor, and the market bar. This picture is what §5's coverage audit was hunting for in the density — and did not find.*
...
| held-out validation | temporal split on the real match; transport beats train-corrected persistence at every horizon (up to +1.44 nats, 91% of anchors, CIs clear of zero); jumps carry a third of the edge; densities conservative in the core with one named thin tail; defender execution calibrated to forecast parity and the decision-signal question answered with a measured "not yet" | more matches — for survival significance, for the press-stratum power, and for everything §6 gates |
